...
/*robdd code. Aufgabe 1*/

     
void Func::Struktur(const ISCAS::Assignments& assignments, ROBDD& robdd){
  for (const ISCAS::Assignment& assignment : assignments) {
     operationenFunc(robdd, std::get<0>(assignment),(ISCAS::Kind)std::get<1>(assignment),std::get<2>(assignment)); 
  }
}

    //Inputs var in map einfügen
    void Func::inputsVariables(ROBDD& robdd, std::vector<std::string> Inputs){
      unsigned i=0;
  ...
  //ite Funktionen 
    const ROBDD::Func& Func::andFunc(ROBDD& robdd,const ROBDD::Func& crArg1, const ROBDD::Func& crArg2) {
      return robdd.ite(crArg1, crArg2, robdd.genFalse());
    }

    const ROBDD::Func& Func::notFunc(ROBDD& robdd, const ROBDD::Func& crArg) {
      return robdd.ite(crArg, robdd.genFalse(), robdd.genTrue());
    }

    const ROBDD::Func& Func::nandFunc(ROBDD& robdd, const ROBDD::Func& crArg1, const ROBDD::Func& crArg2) {
      return robdd.ite(crArg1, robdd.ite(crArg2, robdd.genFalse(), robdd.genTrue()), robdd.genTrue());
    }

    const ROBDD::Func& Func::orFunc(ROBDD& robdd, const ROBDD::Func& crArg1, const ROBDD::Func& crArg2) {
      return robdd.ite(crArg1, robdd.genTrue(), crArg2);
    }

    const ROBDD::Func& Func::xorFunc(ROBDD& robdd, const ROBDD::Func& crArg1, const ROBDD::Func& crArg2) {
      return robdd.ite(crArg1, robdd.ite(crArg2, robdd.genFalse(), robdd.genTrue()), crArg2);
    }

    const ROBDD::Func& Func::norFunc(ROBDD& robdd, const ROBDD::Func& crArg1, const ROBDD::Func& crArg2) {
      return robdd.ite(crArg1, robdd.genFalse(), robdd.ite(crArg2, robdd.genFalse(), robdd.genTrue()));
    }

    const ROBDD::Func& Func::impFunc(ROBDD& robdd, const ROBDD::Func& crArg1, const ROBDD::Func& crArg2) {
      return robdd.ite(crArg1, crArg2, robdd.genTrue());
    }

    const ROBDD::Func& Func::eqFunc(ROBDD& robdd, const ROBDD::Func& crArg1, const ROBDD::Func& crArg2) {
      return robdd.ite(crArg1, crArg2, robdd.ite(crArg2, robdd.genFalse(), robdd.genTrue()));
    }

    //das ist die Funktionen zuwweisung
    void Func::operationenFunc(ROBDD& robdd, const std::string& name, const ISCAS::Kind& kind, const ISCAS::StrVec& Vec) {
...
void Func::drawGraphDot(ROBDD& robdd, const ROBDD::Func& crArg1, std::map<const ROBDD::Func*, int>& Funktionen, int& i, int& node_counter, std::ostream& os) {
  //Else verzweigung
  if (!robdd.isConstant(robdd.getElse(crArg1))) {
    if (Funktionen.count(&robdd.getElse(crArg1)) < 1) {
      ...
      drawGraphDot(robdd, robdd.getElse(crArg1), Funktionen, i, node_counter, os);
    }
  ...
  if (!robdd.isConstant(robdd.getThen(crArg1))) {
    if (Funktionen.count(&robdd.getThen(crArg1)) < 1) {
      ...
      drawGraphDot(robdd, robdd.getThen(crArg1), Funktionen, i, node_counter, os);
    }
...
void Func::printDot(ROBDD& robdd, ISCAS::StrVec str, std::ostream& os) {
  int node_counter = 0;
  ...
  for (std::string& Output : str) {
    ...
    if (Funktionen.find(&m_Inputs->at(Output)) == Funktionen.end()) {
      ...
      if (!robdd.isConstant(m_Inputs->at(Output))) {
        drawGraphDot(robdd, m_Inputs->at(Output), Funktionen, i, node_counter, os);
      }
    }
...
/*hier geht es weiter mit Aufgabe 2 die robdd-sizes*/

//size Berechnen mit counter um den knoten zu zählen

void Func::sizeBerechnen(ROBDD& robdd, const ROBDD::Func& crArg1, std::vector<const ROBDD::Func* >& Funktionen, int& node_counter) {
...
int Func::size(ROBDD& robdd, ISCAS::StrVec inputs, std::map<std::string, const ROBDD::Func&>* FOutputs){
  int counter = 0;
...
void Func::switchup(ISCAS::StrVec &inputs) {
...
//logic
void Func::finalGr(ROBDD &robdd, ISCAS::StrVec &inputs, ISCAS::StrVec &outputs, ISCAS::Assignments &assignments) {
  ...
  while(counter < faktorial) {
    ...
    if(counter != 0) switchup(inputs);
     m_Inputs->clear();
     m_Variablen->clear();
    inputsVariables(robdd, inputs);
    
    Struktur(assignments, robdd);
